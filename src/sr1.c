...
pkt_timer*  deque(pkt_timerQ queue){
...
void removeNode(pkt_timerQ  queue, int acknum){
  ...
  while(currNode != NULL){
    ...
    if(currNode->next != NULL)
    if((currNode->next)->next == NULL && ((currNode->next)->st_packet.seqnum  == acknum)){
      queue->rear = currNode;
    }
...
// The method to remove the node 
  ...
  void remove_node(pkt_timerQ queue,int acknum)
{


    if (queue->front->st_packet.acknum == acknum) /* id matches that of head, let's delete it */
    {
        /* store the queeu head */
      pkt_timer* temp;
        temp = queue->front;
    ...
    else 
    {
      ...
      pkt_timer* curr = queue->front ;
      pkt_timer* prev; 

        while( curr != NULL   )
        {
           if(curr->st_packet.seqnum != acknum){
  ...
  struct msg message;
{
  ...
  if(send_seq_num < (send_base + WINSIZE)){
    ...
    if(A_application == 1 || queuehead == NULL){
      starttimer(0,TIMEOUT);
    }
    printdebug1("\nA_OUTPUT:",&send_pkt,0);
    tolayer3(0,send_pkt);
...

{
   ...
   if(packet.checksum == checksum(&packet)){
    ...
    if(packet.acknum == send_base){
     ...
     calculatetimeout(queuehead,1);
     //stoptimer(0);
     //starttimer(0,newtimeoutval);
     //rmvcalctmt(queuehead,packet.acknum);

    }else{
      
      if(!isqempty(queuehead))
      removeNode(queuehead,packet.acknum);
      //remove from the timer queue
    }
    
    // int winsize_d = 0;
    // //transmit all the remainig packets within the window
    ...
    /* Remove from the queue the packet whose ack is received */
    //removeNode(queuehead,packet);
...
struct pkt packet;
{
 ...
 if(packet.checksum == checksum(&packet)){
   ...
   if(packet.seqnum >= rcvr_base  && packet.seqnum <= rcvr_base + rcvr_winsize - 1){ 
      ...
      if(packet.seqnum == rcvr_base){
            ...
            if(pckt_seqarray[rcvr_base] == rcvr_base){ 

              for(ibuffindx = rcvr_base ;ibuffindx <= min_window;ibuffindx++){ // send the ordered packets to the application
               //if(ibuffindx + 1 < min_window){
                 
                 if(pckt_seqarray[ibuffindx] + 1 == pckt_seqarray[ibuffindx + 1] ){
