...
/* This method calculates the checksum for the packet 
   @ arg1 struct pkt    -- the packet  
...
/* This method used to debug thecode 
   @ arg1 struct pkt    -- the packet  
   @ return int         -- the checksum value
 */

void printdebug(char *func,struct pkt *packet){
...
/* This method makes a packet to be send from sender 
   @ arg1 struct msg    -- the packet  
   @ arg2 int seqnum         -- the checksum value
   @ret struct pkt
 */
...
/* This method makes a packet to be send from receiver 
   @ arg1 int seqnum         -- the checksum value
   @ret struct pkt
 */
  ...
  struct msg message;
{
   ...
   int ipacket_index ;
   A_application += 1;  //Incrementing the packet count
   
   /* If the packet falls inside the window*/
   if(send_seq_num < (send_base + WINSIZE)){
    ...
    tolayer3(0,send_pkt);
    if(send_base == send_seq_num){
      starttimer(0,TIMEOUT);
    }
    send_seq_num++;
   }else{

      /*if window is full then buffer the packets*/
...
{  
   int iPcktIndex ;
   struct pkt currPacket ,send_pkt;
   //printdebug("A_input",&packet);
...
void A_timerinterrupt()
{
  ...
  starttimer(0,TIMEOUT) ;
  //ipacket_num = send_base - 1;ipacket_num < (send_seq_num  -1) -1 ;ipacket_num++){
  /* Retransmitting all the packets that fall within the  window*/
  for(ipacket_num = send_base - 1 ;ipacket_num < (c_index) ;ipacket_num++){
     A_transport++;
     tolayer3(0,packt_buffer[ipacket_num]);
  }
...
/* Initialisinf the sequence number and the base of the window */
void A_init()
{
  ...
  struct pkt packet;
{
   B_transport++ ; 
   struct pkt ack_pkt;
   //printdebug("B_input",&packet);
   if(packet.checksum == checksum(&packet) &&  packet.seqnum == rcvr_expseqnum){
    
      /*If the packet is not corrupt and the packet sequence no is expected then send to application layer */
      B_application++;
      tolayer5(1,packet.payload);
      ack_pkt = make_rpacket(rcvr_expseqnum);
      tolayer3(1,ack_pkt);
      memcpy(&rcvr_ack_packet,&ack_pkt,sizeof(struct pkt));
      rcvr_expseqnum++ ;

   }else{
       
       tolayer3(1,rcvr_ack_packet);
