...
int send_wait_state,rcv_wait_state ;
int s_curr_seqnum ;
int r_curr_seqnum ;
int rcv_exp_seq_num;
int r_expc_ack ;
  ...
  struct msg message;
{
  ...
  struct pkt s_packet ;
  int checksum_val ;
  s_packet.seqnum = send_wait_state ;
  s_packet.acknum = 0 ;
   //copy the data
  strncpy(s_packet.payload,message.data,20);
   //send the packet for checksum calculation;
  checksum_val = checksum(&s_packet);
  s_packet.checksum = checksum_val ;
  memcpy(&prev_packet,&s_packet,sizeof(struct pkt));
  A_transport +=1;
  trnsmit = 1;
  tolayer3(0,s_packet);
  ...
  struct pkt packet;
{
  ...
  int i = 5;
  if(packet.checksum == checksum(&packet) &&  (packet.acknum == send_wait_state)){
     stoptimer(0);
     send_wait_state = togglestate(send_wait_state);
     trnsmit = 0 ;
     printf("THe time packet reached is %f",time);
  ...
  struct pkt packet;
{
  ...
  /* packet not corrupt && ack is expected ACK send the ack*/
  if(packet.checksum == checksum(&packet)){
     ...
     if(packet.seqnum == rcv_wait_state){
     B_application += 1;
     tolayer5(1,packet.payload);
     rcv_wait_state = togglestate(rcv_wait_state);
     }
  ...
  //    ack_packet.seqnum = 0 ;
  //    ack_packet.acknum =  togglestate(rcv_wait_state);
  //    ack_packet.checksum = checksum(&ack_packet);
  //    //memcpy(ack_packet.payload,'0',20);
  //    ack_packet.checksum = checksum(&ack_packet);
